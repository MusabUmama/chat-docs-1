...
from langchain.llms import CTransformers, HuggingFacePipeline, ctransformers, huggingface_pipeline
...
def get_gptq_llm(config: Dict[str, Any]) -> LLM:
    ...
    return huggingface_pipeline.HuggingFacePipeline(pipeline=pipeline)
...
def get_llm(
    ...
    *,
    ...
    if config["llm"] == "ctransformers":
        ...
        llm = ctransformers.CTransformers(callbacks=callbacks, **config)
        ...
        llm = huggingface_pipeline.HuggingFacePipeline.from_model_id(task="text-generation", **config)
    return llm
